...
namespace YektamakDesktop.Formlar.Satis
{
    ...
    /// </summary>
    public partial class SatisSiparisGridForm : Form, IForm
    {
        private static SatisSiparisGridForm _satisSiparisGridForm;
        public static SatisSiparisGridForm satisSiparisGridForm
        {
            get
            {
                if (_satisSiparisGridForm == null)
                {
                    _satisSiparisGridForm = new SatisSiparisGridForm();
                    GlobalData.Yetki(ref _satisSiparisGridForm);
                }
                return _satisSiparisGridForm;
            }
        }
        private List<Control> _controlsToDisable;
        public List<Control> controlsToDisable { get => _controlsToDisable; set => _controlsToDisable = value; }
        public static List<SatisSiparis> satisSiparisList = new List<SatisSiparis>();
        ...
        private DataTable dataTable
        {
            get
            {
                if (_dataTable == null)
                {
                    ...
                    _dataTable = GlobalData.FillDataTable(WebMethods.GetFilteredSatisSiparis, satisSiparisFilter);
        ...
        {
            GlobalData.FillDataGrid(dataTable, dataGridView, satisSiparisFilter);
        }
        private SatisSiparis satisSiparisFilter
        {
            get
            {
                return GlobalData.GridFilter<SatisSiparis>(panelFilter);
            }
        }
        ToolTip buttonFiltreToolTip;
        public SatisSiparisGridForm()
        {
            InitializeComponent();
            SetToolTips();
        ...
        {
            GlobalData.CloseForm(ref _satisSiparisGridForm);
        }
        /// <summary>
        ...
        public void UpdateSatisSiparis(SatisSiparis satisSiparis)
        {
            ...
            if (satisSiparisKayitFormu != null)
            {
                ...
                satisSiparisKayitFormu.Setup(ModeSiparisKayit.UPDATE_FROM_SIPARISGRIDFORM, satisSiparis);
            }
        }
        ...
        private void buttonSatisSiparisEkle_Click(object sender, EventArgs e)
        {
            ...
            {
                satisSiparisKayitFormu.Show();
                satisSiparisKayitFormu.Setup(ModeSiparisKayit.SAVE_FROM_SIPARISGRIDFORM);
            }
        }
        ...
        private void SatisSiparisGridForm_Load(object sender, EventArgs e)
        {
            GlobalData.PlaceFilterFields(dataGridView, panelFilter);
        }
        public void UpdateRow(SatisSiparis satisSiparis)
        {
            int i = GlobalData.IndexOfDataSet(dataTable, satisSiparis.siparisId);
            if (i == -1)
            {
                AddNewRow(satisSiparis);
            }
            else
            {
                GlobalData.UpdateDataRow(dataTable, satisSiparis, i);
            }
        }
        private void AddNewRow(SatisSiparis satisSiparis)
        {
            dataTable.Rows.Add(
                satisSiparis.siparisId,
               satisSiparis.satisProje.projeKod.Id,
               satisSiparis.satisProje.projeKod.kod,
               satisSiparis.satisProje.projeKod.marka.markaId,
               satisSiparis.satisProje.musteri.unvan,
               satisSiparis.siparisTarihi,
               satisSiparis.teslimVadesi,
               satisSiparis.satisTutari.tutar,
               satisSiparis.satisTutari.dovizCinsi.id,
               satisSiparis.kdv.kdvOrani,
               satisSiparis.satisTutari.dovizCinsi.sembol,
               satisSiparis.ongoruMaliyeti.tutar,
               satisSiparis.ongoruMaliyeti.dovizCinsi.id,
               satisSiparis.ongoruMaliyeti.dovizCinsi.sembol
                );
        }
        int oldScrollOffset = 0;
        private void dataGridViewSatisSiparis_Scroll(object sender, ScrollEventArgs e)
        {
            GlobalData.AdjustControlsOnScroll(dataGridView, panelFilter, e, ref oldScrollOffset);
        }
        private void dataGridViewSatisSiparis_ColumnWidthChanged(object sender, DataGridViewColumnEventArgs e)
        {
