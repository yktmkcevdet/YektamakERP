        ...
        private async Task SaveStokKartBatch()
        {
            ...
            foreach (var projeStokKart in projeStokKarts)
            {
                ...
                if (!string.IsNullOrEmpty(existingStokKart?.stokKart.kod))
                {
                    ...
                    {
                        string oldProjeStokKart = JsonConvert.SerializeObject(new StokKart { 
                            kod=existingStokKart.stokKart.kod, 
                            ad=existingStokKart.stokKart.ad,
                            parcaAdi=existingStokKart.stokKart.parcaAdi,
                            aciklama=existingStokKart.stokKart.aciklama,
                            boyut=existingStokKart.stokKart.boyut,
                            malzeme= existingStokKart.stokKart.malzeme
                        });
                        string newProjeStokKart = JsonConvert.SerializeObject(new StokKart
                        {
                            kod = projeStokKart.stokKart.kod,
                            ad = projeStokKart.stokKart.ad,
                            parcaAdi = projeStokKart.stokKart.parcaAdi,
                            aciklama = projeStokKart.stokKart.aciklama,
                            boyut = projeStokKart.stokKart.boyut,
                            malzeme = projeStokKart.stokKart.malzeme
                        });
                        if (oldProjeStokKart != newProjeStokKart)
                        {
                    ...
                    {
                        _projeService.DeleteProjeStokKart(existingStokKart);
                    }
                }
